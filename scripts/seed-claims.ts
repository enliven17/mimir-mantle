/**
 * Seed Claims Script — Creates 15 demo claims across all categories on Mantle.
 *
 * These claims are designed to be immediately resolvable by the oracle agent,
 * demonstrating real traction on the deployed contract.
 *
 * Run AFTER deploying the contract:
 *   npm run seed
 *   (uses MARKET_CREATOR_PRIVATE_KEY by default, override with DEPLOYER_PRIVATE_KEY)
 *
...
if (!PRIVATE_KEY && !DRY_RUN) {
  console.error("MARKET_CREATOR_PRIVATE_KEY (or DEPLOYER_PRIVATE_KEY) required. Use DRY_RUN=1 to preview.");
  process.exit(1);
}
...
async function main(): Promise<void> {
  const stake = mntToWei(STAKE_MNT);

  if (DRY_RUN) {
    console.log("═══════════════════════════════════════════════");
    console.log("  Mimir Seed Claims — DRY RUN");
    console.log(`  ${SEED_CLAIMS.length} claims would be created`);
    console.log(`  Each stake: ${STAKE_MNT} MNT`);
    console.log(`  Total MNT needed: ~${SEED_CLAIMS.length * STAKE_MNT} MNT`);
    console.log("═══════════════════════════════════════════════\n");
  ...
  const walletClient = createMantleWalletClientWithKey(PRIVATE_KEY);
  ...
  console.log(`  Stake/ea : ${STAKE_MNT} MNT`);
  console.log(`  Total    : ~${SEED_CLAIMS.length * STAKE_MNT} MNT`);
  console.log("═══════════════════════════════════════════════\n");

  if (balance < stake * BigInt(SEED_CLAIMS.length)) {
    console.error(`Insufficient balance! Need ~${SEED_CLAIMS.length * STAKE_MNT} MNT, have ${weiToMnt(balance).toFixed(2)} MNT`);
