...
/**
 * Mimir — AI-settled claim market on Mantle.
 *
 * Built for the Mantle Turing Test Hackathon 2026.
 *
 * MNT is the native currency on Mantle (18 decimals).
 ...
 *
 * Agent identity + reputation are tracked off-chain via the Mantle ERC-8004
 * registries. The contract exposes an `agentRole` mapping so the off-chain
 * reputation syncer can identify oracle / market-creator wallets.
 */
contract Mimir {
    // ── State constants ───────────────────────────────────────────────────────
    ...
    // ── ERC-8004 agent registry hooks ─────────────────────────────────────────
    // The off-chain reputation syncer reads these to attribute on-chain wins/
    // losses to specific agent identities registered against Mantle's official
    // ERC-8004 IdentityRegistry. Empty role string = not an agent.
    mapping(address => string)  public agentRole;       // wallet → "oracle" | "market-creator" | ""
    mapping(address => uint256) public agentIdentityId; // wallet → ERC-8004 agentId
    ...
    event AgentRegistered(address indexed wallet, string role, uint256 indexed identityId);
    ...
    // Register an off-chain AI agent against its Mantle ERC-8004 IdentityRegistry
    // tokenId. Role is a free-form tag ("oracle", "market-creator", etc) that
    // off-chain consumers can use to attribute on-chain activity.
    function registerAgent(
        address wallet,
        string calldata role,
        uint256 identityId
    ) external onlyOwner {
        require(wallet != address(0), "Mimir: zero wallet");
        agentRole[wallet]       = role;
        agentIdentityId[wallet] = identityId;
        emit AgentRegistered(wallet, role, identityId);
    }
    ...
    function createClaim(
        ...
        require(msg.value == stakeAmount, "Mimir: wrong MNT value");
    ...
    // Fallback: reject accidental MNT sends without a function call
