...
async function fetchEvents() {
  ...
  if (address === ZERO_ADDRESS) return [] as EventRow[];
    ...
    <main className="mx-auto max-w-[1100px] px-4 py-10 sm:px-6 lg:px-8">
      <header className="mb-8 space-y-1.5">
        ...
        <p className="max-w-2xl text-sm text-pv-muted">
          ...
          row is a real transaction signed by the agent&apos;s own EVM key on
          Mantle. Cached for 20 seconds.
        </p>
      </header>
        ...
        <section className="mb-10 grid gap-4 lg:grid-cols-2">
          <article className="rounded-2xl border border-pv-emerald/35 bg-pv-emerald/[0.05] p-5">
            ...
            <p className="mt-1 text-sm text-pv-text/85">
              Reads expired claims, fetches evidence, asks an LLM, and settles. With auto-challenger on, also stakes MNT on mispriced open claims using Kelly.
            </p>
            <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
              <div>
                <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-pv-emerald/80">Balance</div>
                <div className="mt-0.5 font-display text-base font-bold tabular-nums text-pv-text">{weiToMnt(agentInfo.oracleBal).toFixed(2)} <span className="text-xs text-pv-muted">MNT</span></div>
              </div>
              <div>
          ...
          <article className="rounded-2xl border border-pv-border/40 bg-pv-surface/70 p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <div className="mt-0.5 font-display text-base font-bold tabular-nums text-pv-text">{weiToMnt(agentInfo.ownerBal).toFixed(2)} <span className="text-xs text-pv-muted">MNT</span></div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  ...
                  {e.kind === "challenged" && (
                    <>
                      ...
                      <span className="text-[11px] font-mono text-pv-text/85">{weiToMnt(e.stakeWei).toFixed(2)} MNT</span>
                    </>
                  )}
                  {e.kind === "resolved" && (() => {
