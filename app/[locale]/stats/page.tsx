...
export default async function StatsPage() {
  ...
  const totalWageredMnt = weiToMnt(totalWageredWei);
    ...
    <main className="mx-auto max-w-[1100px] px-4 py-10 sm:px-6 lg:px-8">
      <header className="mb-8 space-y-1.5">
        ...
        <p className="text-sm text-pv-muted">
          Every number on this page is read directly from the Mimir contract on Mantle. Cached for 30 seconds.
        </p>
      </header>
      ...
      <section className="mb-10 grid gap-4 lg:grid-cols-[1.2fr_1fr]">
        ...
        <div className="rounded-2xl border border-pv-border/30 bg-pv-surface/70 p-5 sm:p-6">
          ...
          <p className="mb-5 text-xs text-pv-muted">
            The Mantle wallets that the oracle and market-creator sign with.
          </p>
          {agentInfo ? (
            <div className="space-y-4 text-sm">
              <div>
                <div className="mb-1 flex items-baseline justify-between">
                  ...
                  <span className="font-mono tabular-nums text-pv-text">{weiToMnt(agentInfo.oracleBalance).toFixed(4)} MNT</span>
                </div>
              ...
              <div>
                <div className="mb-1 flex items-baseline justify-between">
                  ...
                  <span className="font-mono tabular-nums text-pv-text">{weiToMnt(agentInfo.ownerBalance).toFixed(4)} MNT</span>
                </div>
              ...
              <p className="border-t border-pv-border/30 pt-3 text-[11px] leading-relaxed text-pv-muted">
                Each agent owns its own EVM private key and signs through viem directly — no custodial layer.
              </p>
            </div>
          ) : (
      ...
      <section className="rounded-2xl border border-pv-border/30 bg-pv-surface/70 p-6">
        <h3 className="mb-4 font-display text-lg font-bold tracking-tight text-pv-text">Get testnet MNT</h3>
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            { label: "Mantle Faucet",  href: "https://faucet.sepolia.mantle.xyz", desc: "Testnet MNT on Mantle Sepolia" },
            { label: "Mantle Explorer",href: "https://explorer.sepolia.mantle.xyz", desc: "Inspect contract activity" },
            { label: "DoraHacks",      href: "https://dorahacks.io/hackathon/mantleturingtesthackathon2026", desc: "Turing Test Hackathon 2026" },
          ].map(({ label, href, desc }) => {
