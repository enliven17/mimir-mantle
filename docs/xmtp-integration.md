...
- `/fund-agents-apps/*` — Gateway / fees; revisit only if required on testnet/mainnet.
...
| [`lib/wallet.tsx`](../lib/wallet.tsx) | Context: `address`, `isConnected`, `connect`, `disconnect`, `error`; uses `window.ethereum`, `eth_requestAccounts`, `ensureMantleChain` (Mantle). |
