...
import { weiToMnt } from "../../../lib/mantle";
...
export function evaluateContrarian(
  ...
  // Need a real imbalance — at least 60% one way.
...
export async function evaluateWhaleWatcher(
  ...
  if (biggestChallenger > claim.creatorStake) {
    return {
      shouldStake: true,
      stakeMnt,
      rationale: `Whale-Watcher: largest individual stake is on the challenger side (${weiToMnt(biggestChallenger).toFixed(2)} MNT vs creator's ${weiToMnt(claim.creatorStake).toFixed(2)}). I follow the whale.`,
    };
  }
  ...
  return {
    shouldStake: false,
    stakeMnt:    0,
    rationale: `Whale-Watcher abstains: the biggest single staker is the creator (${weiToMnt(claim.creatorStake).toFixed(2)} MNT). I can't join the creator side, so I sit out.`,
