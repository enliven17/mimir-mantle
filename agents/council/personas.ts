/**
 * Mimir Council — 10 AI personas that bet on prediction markets (Mantle / MNT).
 *
 * Each persona is an autonomous economic actor with:
 *   - Its own local EVM key (COUNCIL_<SLUG>_PRIVATE_KEY / _ADDRESS)
 *   - A distinct decision-making strategy:
 *       • LLM-biased — uses the configured LLM with a personality prompt prefix
 ...
 *
 * Stakes are paid in native MNT. The contract enforces MIN_STAKE = 2 MNT, so
 * every persona's base stake is floored at 2.
 *
 * The shared persona-runner reads this list and runs each one sequentially to
 * stay under LLM free-tier rate limits.
 */
...
export interface PersonaSpec {
  ...
  /** MNT per stake. Defaults to 2. Floored at the contract MIN_STAKE (2 MNT). */
  stakeMnt?:      number;
...
export const COUNCIL_PERSONAS: PersonaSpec[] = [
  ...
  },
  {
    ...
    archetype:     "specialist",
    categoryFilter: ["sports", "soccer", "nba", "nfl", "tennis", "f1", "worldcup"],
  ...
  },
  {
    ...
    emoji:         "🗣️",
    bio:           "Touches every market. Small stakes, maximum coverage.",
    longBio:       "Bets on everything that crosses its desk, but with minimum stakes. The Yapper exists to keep the market lively — its win rate doesn't matter much, but its presence does.",
    archetype:     "micro",
