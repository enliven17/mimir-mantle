/**
 * Mimir contract client for Mantle (viem-backed).
 *
 * Reads use a singleton public client; writes are signed either through the
 * connected browser wallet (wagmi/viem) or a server-side private key. MNT is
 * the native gas + stake currency (18 decimals).
 */
import {
  createPublicClient,
  createWalletClient,
...
// MIN_STAKE in display MNT (matches Mimir.sol: 2 * 10^18 wei = 2 MNT)
...
export async function readClaimRaw(claimId: number): Promise<ClaimData | null> {
  ...
  try {
    ...
    const creatorStakeMnt = weiToMnt(creatorStakeMicro);
    const totalChStakeMnt = weiToMnt(totalChStakeMicro);
    const reservedMnt     = weiToMnt(reservedLiab);

    const [chAddrs, chStakes] = challengerData;
    const challengers: ClaimChallenger[] = chAddrs.map((addr, i) => {
      const stake   = weiToMnt(chStakes[i]);
...
async function sendBrowserTx(
  ...
  const valueWei = mntToWei(valueMnt);
...
export async function challengeClaim(
  ...
  const result = await sendBrowserTx(
    "challengeClaim",
    [BigInt(claimId), mntToWei(stakeAmount), inviteKey],
    stakeAmount
  );
...
export async function resolveClaim(
  ...
  // Browser resolution is not supported — resolution is oracle-only on Mantle.
...
export async function createRematch(
  ...
  const result = await sendBrowserTx(
    "createRematch",
    [BigInt(parentId), BigInt(params.deadline), mntToWei(params.stake_amount), params.invite_key ?? ""],
    params.stake_amount
  );
  const count = await getClaimCount().catch(() => null);
  return { ...result, claimId: count };
...
export async function executeDemoWrite(
  ...
  if (action === "resolve_claim") {
    ...
    throw new Error(`Claim ${claimId}: use the oracle agent to resolve on Mantle.`);
  }
  ...
  if (action === "create_rematch") {
    ...
    const result = await sendServerTx(
      ...
      [BigInt(parentId), BigInt(deadline), mntToWei(stake_amount), invite_key],
      stake_amount
    );
    const count = await getClaimCount().catch(() => null);
    return { ...result, claimId: count };
...
/**
 * On Mantle, resolution is handled by the off-chain oracle agent automatically.
...
/** Kept for UI compatibility — no-op on Mantle. */
export async function resetVSResolveRequest(
  _wallet: string,
  _claimId: number,
  _inviteKey = ""
): Promise<ClaimWriteResult> {
  throw new Error("Not applicable on Mantle — oracle resolves automatically.");
}
